...
Event-tap live fire is `capture.selection` Shift double-tap. Other seeded globals persist through native registration without an OS grab. App-local chords and recorded modifier multi-taps persist for the UI resolver; Quick Panel keydown still uses hardcoded composer chords.
