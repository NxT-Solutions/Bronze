...
Menu-bar or Bronze-menu Capture, and Shift double-tap, persist AX selected text into the same SQLite store as the composer. Every `ShortcutActionId` ships an enabled default (`docs/12` §4). Capture does not steal source focus and does not force-show the panel.
