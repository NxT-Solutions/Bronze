...
- `items.title` is nullable TEXT (migration `items_title`). Persist, composer add, and body edit write `compact_title` first (40-character word-boundary clamp, no ellipsis glyph). A later refine may replace that string from `general.titleModel`: extractive stops; a bundled pin or imported GGUF uses the local worker; Ollama or a hosted provider (ADR-021 / ADR-022) that cannot answer uses the bundled engine for this Mac, then `compact_title`. `clean_title` strips a leaked `Title:` prefix. A local failure never calls a hosted API (ADR-019 Proposed). Existing rows keep their stored string until recapture or edit.
