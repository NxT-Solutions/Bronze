...
- Persist read (`read_capture_selection`) prefers a Bronze / own-PID selection when Bronze is frontmost and has one. Otherwise it reads the snapshotted last-external PID. Menu Capture uses the same source rule.
- System-wide focused AX runs when no last-external PID exists. Own PID and Bronze names are valid AX targets on that fallback.
...
2. Resolve target PID (`read_capture_selection`): Bronze / own PID when Bronze is frontmost and has a selection; otherwise last-external; otherwise system-wide focused AX. Own PID and Bronze names are valid AX targets. Last-external memory still ignores Bronze.
