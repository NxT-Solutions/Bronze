...
- changing UI language persists `SettingsV1.general.locale` (en, nl, fr, de, es, it, ru, uk, hr, sl, da, sv, nb, fi, tr) and applies the catalog live in open WebViews, including `html lang`/`dir`. Language options use endonyms with `lang` on each option. Item bodies stay `lang="und" dir="auto"` and are never relabeled. Native app and status menus follow the persisted locale at launch; a live switch does not rebuild the app menu. User content, focus, and the active task stay put.
- changing Title engine persists `SettingsV1.general.titleModel` (`extractive`, `smol-135`, `smol-360`, `qwen-05`, `custom`, `ollama`, `hosted-openai`, `hosted-anthropic`, `hosted-openrouter`) and reloads the local title worker without relaunch when the choice is extractive, a pin, or custom. `#title-model-status` (`role=status`, `aria-live=polite`) shows catalog idle, loading, hashing, ready, missing, or failed copy. A `span.title-engine-spinner` next to the select is visible only while loading or hashing (`aria-busy` on the status, not the select). `#title-model-progress` sits under that status. It is determinate while the worker reports `bytesRead` and `bytesTotal`, and indeterminate while loading or hashing with no byte count. Ready, failed, missing, and idle remove it. The progress name is the visible status text. Focus stays on the select. Reduce Motion (`html[data-motion="reduce"]` / `data-reduce-motion` / `prefers-reduced-motion`) stops the decorative spin and the indeterminate animation; a determinate bar stays static and shows the percent text. A missing bundled file says the model is not in this build. There is no download control. Import GGUF stays the outlined button. Failed reasons are catalog strings (`bad_hash`, `timeout`, `unreadable`) with no huge paths. Custom import shows name and byte size, never a path. Ollama lists models only when that row is selected. Hosted rows show host and payload class before the first send; the key field is write-only. Local and Ollama copy says text stays on this Mac. Hosted copy names the host. Queue chrome does not show this status. This is Settings markup, not a WCAG or VoiceOver claim.
- changing Reduce motion persists `SettingsV1.general.reduceMotion` (`system` | `on` | `off`, default `system`) and sets `document.documentElement.dataset.motion` to `full` or `reduce` live in Settings and the queue (`ui-motion-changed`). `#reduce-motion` is a real labeled select; help is “Follows this Mac unless you override it.” `system` follows this Mac. `on` always reduces. `off` plays animations even when macOS Reduce Motion is on. Reset field returns `system`. No restart.
- changing Launch at login persists `SettingsV1.general.launchAtLogin` (default off) and applies `SMAppService.mainApp`. `#launch-at-login` is a real labeled checkbox with circled-i info. Help states it starts Bronze at login and stays off until turned on. `[data-login-item-status]` shows catalog `enabled` / `not_registered` / `requires_approval` / `unavailable` from `login_item_status`. A requires-approval or unavailable status does not uncheck the box. Reset field returns off. Unbundled debug cannot register. This is Settings markup, not a WCAG or VoiceOver claim.

Exported settings always exclude app credentials, permission tokens, diagnostic events, and machine paths. Diagnostics use a separate previewed support-file flow: Help shows a visible English `bronze-support.txt` body (versions, machine, redacted path, permissions, queue counts, last-hour events) that matches Export Support File. Bronze never uploads it. This is Help markup, not a WCAG or VoiceOver claim. The Settings Export preview card is a live preview of the pending file: it lists included categories, then names leftover user-entered profile literals, excluded-app policies, and custom shortcuts that may themselves be sensitive. Export is the one filled primary on Settings; Import is ghost; Help stays ghost trailing. Both actions open a rust-owned save/open panel. The WebView never supplies a filesystem path. Status after write/read is Exported or Imported (or a catalog failure), not a machine path. This file is not the Library archive.
