...
- Inbox `#capture-status` shows catalog `capture.announce.saved|rejected|denied|protected|failed` after every persist outcome. A saved row also shows catalog `capture.source` (`From {appName}`) and a 16px official source-app icon when Rust can resolve the local `.app`. Composer **Add** rows stay unlabeled. Each row may show a stored `title` heading; the body is sanitized constrained markdown (including line-start and `•` lists) with **Show more** / **Show less** when it overflows. Copy writes plain plus sanitized HTML and reports catalog `copy.announce.copied` / `copy.announce.failed`. Titles persist as `compact_title` and may refine on-device (ADR-019 Proposed; no hosted AI, no PCC, no bundled GGUF)
