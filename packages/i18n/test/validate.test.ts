...
describe("extractPlaceholders", () => {
  it("takes only top-level ICU argument names", () => {
    expect(
      extractPlaceholders(
        "{count, plural, =0 {0 items} one {1 item} other {# items}}",
      ),
    ).toEqual(["count"]);
    expect(extractPlaceholders("plain {foo} and {bar}")).toEqual([
      "bar",
      "foo",
    ]);
  });
...
describe("collectCatalogErrors", () => {
  ...
  it("reports missing keys", () => {
    ...
    expect(
      errors.some(
        (e) => e.includes("missing keys") && e.includes("queue.count"),
      ),
    ).toBe(true);
  });
  ...
  it("fails when ICU plural collapses to a placeholder", () => {
    ...
    try {
      ...
      expect(
        errors.some(
          (e) =>
            e.includes("ICU structure lost") ||
            e.includes("placeholder mismatch"),
        ),
      ).toBe(true);
    } finally {
      rmSync(dir, { recursive: true });
...
describe("committed pseudo catalogs", () => {
  it("match the generators", () => {
    const en = JSON.parse(
      readFileSync(resolve(PKG, "locales/en/app.json"), "utf8"),
    ) as Record<string, string>;
    const xa = JSON.parse(
      readFileSync(resolve(PKG, "locales/en-XA/app.json"), "utf8"),
    ) as Record<string, string>;
    const xb = JSON.parse(
      readFileSync(resolve(PKG, "locales/ar-XB/app.json"), "utf8"),
    ) as Record<string, string>;
