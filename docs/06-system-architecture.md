...
The quick panel loads its overview (`queued`, `copied`, `active`) with `queue_query(cursor, filter, limit)`. The first call omits `cursor`. The page is `{ items, nextCursor }`. `nextCursor` is null when no further row sorts after the page. The cursor is the opaque keyset `v1.` + hexadecimal UTF-8 `rank` + `.` + hexadecimal UTF-8 `id`, in `ORDER BY rank, id`. When `sort` is `newest` or `oldest`, every page uses `ORDER BY created_at_ms, id` in that direction, and the cursor is `v2.{sort}.{created_at_ms}.{hex id}`. A cursor for a different sort is ignored and the query returns the first page. It is not an SQL offset. Page size is 20, and a larger `limit` is clamped to 20 (QUE-002, SEC-002). An invalid cursor fails as `queue_cursor_invalid`. A row whose rank sorts before that keyset does not appear on the next page and does not drop rows after it. The panel requests the next page when the end sentinel nears the scrollport, when keyboard focus enters the last row, and when ArrowDown, End, or PageDown is pressed on that row (QUE-008, A11Y-001). `queue_page_for_item` walks the same keyset until the id is included. The loading sentence is the catalog key `queue.list.loading` (I18N-001).
