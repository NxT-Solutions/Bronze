...
- Toast/status: `role=status`/polite region for routine result; alert only for urgent blocking error. Capture terminals also post a Notification Center banner (`UNUserNotificationCenter` in a `.app`, or signed `BronzeNotice.app` in unbundled `tauri dev`) and viewport-fixed `#chrome-notice`; `#capture-status` stays visually hidden. Critical actions persist outside toast.
