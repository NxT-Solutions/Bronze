...
P0 stores constrained markdown when AX attributes exist, else exact Unicode plaintext. It preserves Unicode scalar content, line breaks, indentation, and intentional leading/trailing whitespace. No trim, NFC/NFD rewrite, translation, smart-quote conversion, remote lookup, or hosted model runs on the AX path. Constrained markdown is produced from the attributed selection, not by parsing the user's text as a document language. Bold and italic come from `AXFontBold` / `AXFontItalic` or a bold/italic font name (`**` / `*`). `AXHighlight`, or a background color that differs from the selection's shared background, becomes an inline code span (`` ` ``). A uniform background stays plain text.
...
- On native runtime start, after the app can present system UI and before the event tap is created, requests Accessibility (`AXIsProcessTrustedWithOptions` with `kAXTrustedCheckOptionPrompt`) and Input Monitoring (`CGRequestListenEventAccess`) when that capability is not already granted and this binary has not shown the prompt. The shown prompt is remembered for that binary. A later launch does not call the request APIs again. Requests run off the event-tap callback thread.
- On the first capture path, snapshots last-external PID first (`note_external_focus` / `bronze_native_frontmost_pid`), then requests a used permission only when it is still ungranted and this binary has not already shown that prompt. Launch is not a prompt loop. The permission dialog is never shown before the PID snapshot.
- Permission-health Retest always calls those request APIs again, and also re-reads the no-prompt preflight (`AXIsProcessTrusted`, `CGPreflightListenEventAccess`). A live preflight grant stays granted when the request call returns false. macOS may refuse a second Input Monitoring dialog; Accessibility may re-prompt on some OS versions.
- If the OS will not re-prompt after that attempt, a System Settings deep-link is the fallback — after the request, not instead of it. Opening Accessibility or Input Monitoring also reveals the running app in Finder and names that copy (display name, version, bundle path). An ad-hoc signature’s cdhash changes on every build, so a switch that is already on can belong to another copy of the same bundle id. Bronze never auto-opens Settings and never deep-links Screen Recording.
...
- Capture persists AX selection from other apps into the same queue store as the composer: `AXSelectedTextRange` plus `AXAttributedStringForRange` when available, else `AXSelectedText` (CFAttributedString or CFString), else `AXStringForRange`. Attributed runs become constrained markdown; fallback is exact Unicode plaintext. Persist writes `compact_title` immediately (40-character word-boundary clamp, no ellipsis glyph) and emits `capture-result` `{ terminal, reason }` for every persist outcome (`saved`/`rejected`/`failed`/`cancelled`) and `queue-changed` on Saved. A Rust `llama-cpp-2` worker may then refine the stored title from Settings `general.titleModel` (extractive `compact_title`, a hash-pinned bundled GGUF, or an imported GGUF). Ollama or a hosted provider that cannot answer uses the bundled engine for this Mac, then `compact_title`. Missing file, hash mismatch, timeout, empty, or ungrounded output keeps `compact_title` (ADR-019 Proposed). A local failure never calls a hosted API. Settings listens for `title-engine-status` and `title_engine_status` from that worker (`{ tier, phase, reason }`); the event-tap thread does not emit them. It does not reveal or focus the Quick Panel (WIN-003 capture-only).
...
8. At each allowed node (`AXTextField`, `AXTextArea`, `AXStaticText`, `AXWebArea`, `AXBrowser`, `AXDocument`, `AXText`, `AXComboBox`) and at a neutral container that exposes selected text (common on Electron/Chromium hosts such as GitKraken), prefer attributed selection: `AXSelectedTextRange` plus `AXAttributedStringForRange`. Bold and italic come from `AXFontBold` / `AXFontItalic` inside `AXFont`, or from `AXFontName` / `AXFontFamily` when the face name carries the trait, and become constrained markdown `**` / `*`. `AXHighlight`, or `AXBackgroundColor` when that color is not the selection's shared background, becomes an inline code span. A uniform background is not a mark. Empty or missing child value is inconclusive; continue to children, then ancestor.
